...
class AkkaOrderBookResponseHandler(delegators: List[Handler[OrderBookResponse]]) extends AkkaHandler[OrderBookResponse] {
  override def actorRefHandlers: List[ActorRef] = delegators.map(handler => actorSystem.actorOf(Props(new AkkaHandlerWrapper[OrderBookResponse](handler)).withRouter(RoundRobinRouter(3))))
}
