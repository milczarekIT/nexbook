...
object FixOrderConverter {
  ...
  def convert(fixOrder: NewOrderSingle): NewOrder = fixOrder.getOrdType.getValue match {
    case OrdType.LIMIT => new NewLimitOrder(fixOrder.getClOrdID.getValue, fixOrder.getSymbol.getValue, fixOrder.getAccount.getValue, resolveSide(fixOrder.getSide), fixOrder.getOrderQty.getValue, fixOrder.getPrice.getValue, senderCompId(fixOrder))
    case OrdType.MARKET => new NewMarketOrder(fixOrder.getClOrdID.getValue, fixOrder.getSymbol.getValue, fixOrder.getAccount.getValue, resolveSide(fixOrder.getSide), fixOrder.getOrderQty.getValue, senderCompId(fixOrder))
  }
