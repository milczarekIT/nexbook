...
class AkkaOrderChangeHandler(delegators: List[Handler[OrderChangeCommand]]) extends AkkaHandler[OrderChangeCommand] {
  override def actorRefHandlers: List[ActorRef] = delegators.map(handler => actorSystem.actorOf(Props(new AkkaHandlerWrapper[OrderChangeCommand](handler)).withRouter(RoundRobinRouter(6))))
}
