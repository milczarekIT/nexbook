...
class AkkaNewOrderHandler(delegators: List[Handler[NewOrder]]) extends AkkaHandler[NewOrder] {
  ...
  override def actorRefHandlers: List[ActorRef] = delegators.map(handler => actorSystem.actorOf(Props(new AkkaHandlerWrapper[NewOrder](handler)).withRouter(RoundRobinRouter(8))))
}
