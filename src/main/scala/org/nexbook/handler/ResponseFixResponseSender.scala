...
class ResponseFixResponseSender extends ResponseHandler {
  ...
  trait ProcessingResponseFixMessageConverter[T <: OrderProcessingResponse] {
    ...
    def convert(response: OrderProcessingResponse): Option[(Message, SessionID)] = doConvert(response.asInstanceOf[T])
  ...
  class DefaultProcessingResponseFixMessageConverter extends ProcessingResponseFixMessageConverter[OrderProcessingResponse] {
    override protected def doConvert(response: OrderProcessingResponse): Option[(Message, SessionID)] = None
  }
